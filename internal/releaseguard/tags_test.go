...
// the tag-reacting providers/go release workflow must never collide with
// the root release line's v* tag namespace, and the root release workflow's
// `git describe --tags` calls must never resolve a providers/go/vX.Y.Z tag
// as if it were a root version (design.md D6's "blast radius on the root
// release line"). It parses both workflow YAMLs as text/YAML, the same way
// internal/daggerpin and internal/workspaceguard guard other design
// decisions, and never requires a running Dagger engine or a real GitHub
// Actions run.
...
// (b) release.yml's tag globs don't match providers/go/v0.1.0.
func TestReleaseYAML_TagGlobsExcludeProviderNamespace(t *testing.T) {
	...
	for _, g := range globs {
		if GlobMatches(g, "providers/go/v0.1.0") {
			t.Fatalf("release.yml's tag glob %q matches %q, want disjoint from providers/go/* (design.md D6)", g, "providers/go/v0.1.0")
		}
	}
}
