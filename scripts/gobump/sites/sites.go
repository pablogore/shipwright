// Package sites implements the literal-replacement half of the coordinated
// Go SDK/toolchain version bump (issue #280, design.md D-4/D-5/D-6): an
// explicit, enumerated registry of every plain-string Go version pin the
...
// Site declares one anchored literal-replacement location whose Go version
// substring must track the repository's canonical toolchain version.
type Site struct {
...
// Sites is the production literal-replacement registry (issue #280,
// design.md D-5): every plain-string Go version pin that
// GoRuntimeUpgrader's go.mod/go.work/.go-version handling cannot reach.
// Populated from a fresh, full-repo audit (not the design doc's prior
// list, which three earlier SDD phases each found incomplete) and cross-
// checked against D-7's exclusions: nothing under
// providers/go/testdata/runtime/**, and .dagger/release_test.go and
// testing/integration/go/integration_test.go stay deliberately
// unregistered (they assert behavior, not the repo's pin).
//
// Decision (tasks.md 1.2): the illustrative doc-comment literals in
// internal/pipelines/shared/builder.go, internal/pipelines/shared/tests.go,
// and internal/pipelines/test/gotester.go ARE registered. They are cheap
// to keep in sync, each one currently restates the real fallback default
// used a few lines below it (not an arbitrary format example), and a
// removed/reformatted comment is caught loudly by E2 rather than silently
// going stale.
//
// Two categories found during the census that were deliberately left
// OUT of this registry, because their literals do not track "the"
// current pin:
...
//
// Newly found during this pass, absent from every prior SDD phase's list:
// internal/executors/docker_executor.go's goVersion fallback (identical
// pattern to the three already-known fallback sites), and the exact set
// of anchors within docs/ANALYSIS_AND_RECOMMENDATIONS.md and
// docs/PRODUCTION_DEPLOYMENT.md (design.md's open question named the
// files; this registry is the first pass to enumerate their anchors).
var Sites = []Site{
