...
Each step MUST declare exactly one `capability` field naming one of the five
capability interfaces defined by `public-module-api` (Build, Test, Artifact,
Deploy, Run), and exactly one `uses` field identifying either a local
provider (`provider` + `version`) or an external provider (`module` +
`version`). A step missing `uses`, or naming a `capability` outside the five
defined interfaces, MUST fail schema validation.

#### Scenario: Step with capability and uses validates

- GIVEN a step `{id: build, capability: build, uses: {provider: maven,
  version: "1"}}`
