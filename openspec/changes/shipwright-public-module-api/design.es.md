...
Además, se mantiene de la Revisión 1: las credenciales cruzan el contrato público únicamente como `*dagger.Secret` (una credencial en texto plano en la superficie garantizada falla el test golden de superficie), y el enrutamiento de pasos/capacidades DEBE fallar cerrado — un id de paso desconocido o una capacidad ausente devuelven error, nunca un salto silencioso.
