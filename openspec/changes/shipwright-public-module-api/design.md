...
Additionally carried from Revision 1: credentials cross the public contract as `*dagger.Secret` only. The golden surface test is a textual diff, not a semantic type check — it does not automatically reject a plaintext credential field on its own; it forces any exported-surface change, including one, into a reviewable `-update` diff that a human must read and deliberately accept, never blind-accept (`pkg/shipwright/api_golden_test.go`'s own doc comment states this). The actual guarantee is that discipline, not an automated rule. Step/capability routing MUST fail closed — an unknown step id or an absent capability returns an error, never a silent skip.
